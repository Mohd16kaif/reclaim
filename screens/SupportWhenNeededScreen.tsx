...
type RootStackParamList = { SupportWhenNeeded: any; InstantUrgeProtection: any; };
type ScreenNavigationProp = StackNavigationProp<RootStackParamList, "SupportWhenNeeded">;
const { width } = Dimensions.get("window");
const SupportWhenNeededScreen: React.FC = () => {
  const navigation = useNavigation<ScreenNavigationProp>();
  const route = useRoute<RouteProp<RootStackParamList, "SupportWhenNeeded">>();
  ...
  const handleNextPress = () => { (navigation as any).navigate("InstantUrgeProtection", { ...params }); };
      ...
      <SafeAreaView style={styles.safeArea} edges={["top", "bottom"]}>
        <View style={styles.content}>
          ...
          <Animated.View style={[styles.phoneSection, { opacity: phoneOpacity, transform: [{ scale: phoneScale }] }]}>
            <Image source={require('../assets/images/support-when-needed.png')} style={styles.phoneImage} resizeMode="contain" />
          </Animated.View>
          <Animated.View style={[styles.footerSection, { opacity: bodyOpacity }]}>
            <Text style={styles.heading}>Support When Needed</Text>
            <View style={styles.bodySection}><Text style={styles.bodyText}>Calm guidance is available during moments when staying focused feels difficult.</Text></View>
...
export default SupportWhenNeededScreen;
