import { RouteProp, useNavigation, useRoute } from "@react-navigation/native";
import { StackNavigationProp } from "@react-navigation/stack";
import React, { useEffect, useRef, useState } from "react";
import {
    Animated,
    StyleSheet,
    Text,
    TouchableOpacity,
    View,
} from "react-native";
import { SafeAreaView } from "react-native-safe-area-context";

type RootStackParamList = {
  Splash: undefined;
  Welcome: undefined;
  OnboardingQuestion: {
    questionNumber?: number;
    totalQuestions?: number;
  };
  GenderSelection: {
    questionNumber?: number;
    totalQuestions?: number;
    userName?: string;
  };
  DateOfBirthPicker: {
    questionNumber?: number;
    totalQuestions?: number;
    userName?: string;
    selectedGender?: string;
  };
  Home: undefined;
  SignIn: undefined;
};

type GenderSelectionNavigationProp = StackNavigationProp<
  RootStackParamList,
  "GenderSelection"
>;

type GenderSelectionRouteProp = RouteProp<
  RootStackParamList,
  "GenderSelection"
>;

const ANIMATION_DURATION = 400;
const DEFAULT_TOTAL_QUESTIONS = 25;

type GenderOption = "Male" | "Female" | "Other";

const GenderSelectionScreen: React.FC = () => {
  const navigation = useNavigation<GenderSelectionNavigationProp>();
  const route = useRoute<GenderSelectionRouteProp>();
  const progressAnim = useRef(new Animated.Value(0)).current;
  const [selectedGender, setSelectedGender] = useState<GenderOption | null>(
    null
  );

  const questionNumber = route.params?.questionNumber ?? 2;
  const totalQuestions =
    route.params?.totalQuestions ?? DEFAULT_TOTAL_QUESTIONS;
  const userName = route.params?.userName ?? "";

  const progress = Math.min(
    1,
    Math.max(0, questionNumber / totalQuestions || 0)
  );

  useEffect(() => {
    Animated.timing(progressAnim, {
      toValue: progress,
      duration: ANIMATION_DURATION,
      useNativeDriver: false,
    }).start();
  }, [progress, progressAnim]);

  const handleBack = () => {
    navigation.goBack();
  };

  const handleGenderSelect = (gender: GenderOption) => {
    setSelectedGender(gender);
  };

  const handleContinue = () => {
    if (!selectedGender) return;

    // Navigate to date of birth picker (question 3)
    navigation.navigate("DateOfBirthPicker", {
      questionNumber: 3,
      totalQuestions: totalQuestions,
      userName: userName,
      selectedGender: selectedGender,
    });
  };

  const progressWidth = progressAnim.interpolate({
    inputRange: [0, 1],
    outputRange: ["0%", "100%"],
  });

  const genderOptions: GenderOption[] = ["Male", "Female", "Other"];

  return (
    <SafeAreaView style={styles.safeArea} edges={["top", "left", "right"]}>
      <View style={styles.container}>
        {/* Header with back button + progress bar on one line */}
        <View style={styles.headerRow}>
          <TouchableOpacity
            style={styles.backButton}
            activeOpacity={0.7}
            onPress={handleBack}
          >
            <Text style={styles.backArrow}>{"←"}</Text>
          </TouchableOpacity>

          <View style={styles.progressTrack}>
            <Animated.View
              style={[styles.progressFill, { width: progressWidth }]}
            />
          </View>
        </View>

        {/* Main content */}
        <View style={styles.content}>
          <Text style={styles.heading}>Choose your Gender</Text>
          <Text style={styles.subheading}>
            This will be used to calibrate your custom plan
          </Text>

          <View style={styles.optionsContainer}>
            {genderOptions.map((gender) => {
              const isSelected = selectedGender === gender;
              return (
                <TouchableOpacity
                  key={gender}
                  style={[
                    styles.genderButton,
                    isSelected && styles.genderButtonSelected,
                  ]}
                  activeOpacity={0.8}
                  onPress={() => handleGenderSelect(gender)}
                >
                  <Text
                    style={[
                      styles.genderButtonText,
                      isSelected && styles.genderButtonTextSelected,
                    ]}
                  >
                    {gender}
                  </Text>
                </TouchableOpacity>
              );
            })}
          </View>
        </View>

        {/* Bottom button */}
        <View style={styles.footer}>
          <TouchableOpacity
            style={[
              styles.continueButton,
              !selectedGender && styles.continueButtonDisabled,
            ]}
            activeOpacity={0.85}
            onPress={handleContinue}
            disabled={!selectedGender}
          >
            <Text style={styles.continueButtonText}>Continue</Text>
          </TouchableOpacity>
        </View>
      </View>
    </SafeAreaView>
  );
};

export default GenderSelectionScreen;

const styles = StyleSheet.create({
  safeArea: {
    flex: 1,
    backgroundColor: "#FFFFFF",
  },
  container: {
    flex: 1,
    backgroundColor: "#FFFFFF",
  },
  headerRow: {
    flexDirection: "row",
    alignItems: "center",
    paddingHorizontal: 24,
    paddingTop: 8,
    paddingBottom: 12,
  },
  backButton: {
    width: 50,
    height: 50,
    borderRadius: 25, // Change from 16 to 25 (half of 50 for perfect circle)
    backgroundColor: "#F5F3FF",
    alignItems: "center",
    justifyContent: "center",
  },

  backArrow: {
    fontSize: 20,
    color: "#000000",
  },
  progressTrack: {
    flex: 1,
    height: 3,
    backgroundColor: "#E5E5E5",
    marginLeft: 12,
    borderRadius: 2,
    overflow: "hidden",
  },
  progressFill: {
    height: "100%",
    backgroundColor: "#000000",
  },
  content: {
    flex: 1,
    paddingHorizontal: 24,
    paddingTop: 0, // Same as OnboardingQuestionScreen
  },
  heading: {
    fontSize: 28,
    alignSelf: "center", // Centered like OnboardingQuestionScreen
    fontWeight: "700",
    color: "#000000",
    marginBottom: 8,
  },
  subheading: {
    fontSize: 14,
    fontWeight: "400",
    color: "#A1A1AA", // Same as input placeholder color
    textAlign: "center",
    marginBottom: 24,
  },
  optionsContainer: {
    gap: 12,
  },
  genderButton: {
    height: 56, // Same height as input field
    borderRadius: 16,
    alignItems: "center",
    justifyContent: "center",
    backgroundColor: "#F9FAFB", // Same as "Type Here" input box
    borderWidth: 0, // No border
  },
  genderButtonSelected: {
    backgroundColor: "#000000", // Full black when selected
  },
  genderButtonText: {
    fontSize: 16,
    fontWeight: "600",
    color: "#000000",
  },
  genderButtonTextSelected: {
    color: "#FFFFFF",
  },
  footer: {
    paddingHorizontal: 24,
    paddingTop: 20,
    paddingBottom: 32,
    backgroundColor: "#FFFFFF",
    shadowColor: "#000",
    shadowOffset: { width: 0, height: -2 },
    shadowOpacity: 0.1,
    shadowRadius: 8,
    elevation: 5,
  },
  continueButton: {
    backgroundColor: "#000000",
    borderRadius: 999,
    paddingVertical: 18,
    alignItems: "center",
    justifyContent: "center",
  },
  continueButtonDisabled: {
    backgroundColor: "#E0E0E0",
  },
  continueButtonText: {
    color: "#FFFFFF",
    fontSize: 16,
    fontWeight: "600",
    letterSpacing: 0.3,
  },
});
