...
type UrgeIntensityNavigationProp = StackNavigationProp<
  RootStackParamList,
  "UrgeIntensity"
>;

type UrgeIntensityRouteProp = RouteProp<RootStackParamList, "UrgeIntensity">;
...
const UrgeIntensityScreen: React.FC = () => {
  const navigation = useNavigation<UrgeIntensityNavigationProp>();
  const route = useRoute<UrgeIntensityRouteProp>();
  const progressAnim = useRef(new Animated.Value(0)).current;
  const [selectedIntensity, setSelectedIntensity] = useState<string | null>(null);

  const questionNumber = route.params?.questionNumber ?? 19;
  ...
  const handleOptionSelect = (option: string) => {
    setSelectedIntensity(option);
  };

  const handleContinue = () => {
    if (!selectedIntensity) return;

    // Navigate to UrgeLoop screen (question 20)
    navigation.navigate("UrgeLoop", {
      questionNumber: 20,
...
export default UrgeIntensityScreen;
