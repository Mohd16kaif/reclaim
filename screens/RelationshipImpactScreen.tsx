...
type RelationshipImpactNavigationProp = StackNavigationProp<
  RootStackParamList,
  "RelationshipImpact"
>;

type RelationshipImpactRouteProp = RouteProp<
  RootStackParamList,
  "RelationshipImpact"
>;
...
type ImpactOption = "Not really" | "A little" | "A lot" | "I'm not sure" | "Prefer not to say";
...
const RelationshipImpactScreen: React.FC = () => {
  const navigation = useNavigation<RelationshipImpactNavigationProp>();
  const route = useRoute<RelationshipImpactRouteProp>();
  const progressAnim = useRef(new Animated.Value(0)).current;
  const [selectedImpact, setSelectedImpact] = useState<ImpactOption | null>(
    null
  );

  const questionNumber = route.params?.questionNumber ?? 15;
  ...
  const handleOptionSelect = (option: ImpactOption) => {
  ...
  const handleContinue = () => {
    ...
    // Navigate to RelationshipBenefitsInfo screen (question 16)
    navigation.navigate("RelationshipBenefitsInfo", {
      questionNumber: 16,
      ...
      <View style={styles.container}>
        ...
        <View style={styles.content}>
          ...
          <Text style={styles.heading}>
            Has porn affected your real-life relationships?
          </Text>
          ...
          <View style={styles.optionsContainer}>
            {impactOptions.map((option) => {
              const isSelected = selectedImpact === option;
              return (
                <TouchableOpacity
                  key={option}
                  style={[
        ...
        <View style={styles.footer}>
          <TouchableOpacity
            style={[
              ...
              !selectedImpact && styles.continueButtonDisabled,
            ]}
            activeOpacity={0.85}
...
export default RelationshipImpactScreen;
