...
type RootStackParamList = { InstantUrgeProtection: any; LeaveRating: any; };
type ScreenNavigationProp = StackNavigationProp<RootStackParamList, "InstantUrgeProtection">;
const { width } = Dimensions.get("window");
const InstantUrgeProtectionScreen: React.FC = () => {
  const navigation = useNavigation<ScreenNavigationProp>();
  const route = useRoute<RouteProp<RootStackParamList, "InstantUrgeProtection">>();
  ...
  const handleNextPress = () => { (navigation as any).navigate("LeaveRating", { ...params }); };
        ...
        <View style={styles.content}>
          ...
          <View style={styles.phoneSection}>
            <Animated.View style={[styles.phoneContainer, { opacity: phoneOpacity, transform: [{ scale: phoneScale }] }]}>
              <Image source={require("../assets/images/instant-urge-protection.png")} style={{ width: "100%", height: "100%" }} resizeMode="contain" />
            </Animated.View>
          </View>
          <Animated.View style={[styles.footerSection, { opacity: bodyOpacity }]}>
            <Text style={styles.heading}>Instant Urge Protection</Text>
            <View style={styles.bodySection}><Text style={styles.bodyText}>When urges intensify, you can activate temporary protection to lock access and regain control.</Text></View>
...
export default InstantUrgeProtectionScreen;
