...
type SessionDurationNavigationProp = StackNavigationProp<
  RootStackParamList,
  "SessionDuration"
>;

type SessionDurationRouteProp = RouteProp<
  RootStackParamList,
  "SessionDuration"
>;
...
const SessionDurationScreen: React.FC = () => {
  const navigation = useNavigation<SessionDurationNavigationProp>();
  const route = useRoute<SessionDurationRouteProp>();
  const progressAnim = useRef(new Animated.Value(0)).current;
  const [selectedDuration, setSelectedDuration] = useState<string | null>(
    null
  );

  const questionNumber = route.params?.questionNumber ?? 11;
  ...
  const handleContinue = () => {
    ...
    // Navigate to reclaim results info screen (question 12)
    navigation.navigate("ReclaimResultsInfo", {
      questionNumber: 12,
      ...
      <View style={styles.container}>
        ...
        <View style={styles.content}>
          <Text style={styles.heading}>
            On average, how long do you watch per session?
          </Text>
...
export default SessionDurationScreen;
