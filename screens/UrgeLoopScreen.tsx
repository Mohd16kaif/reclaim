...
type UrgeLoopNavigationProp = StackNavigationProp<
  RootStackParamList,
  "UrgeLoop"
>;

type UrgeLoopRouteProp = RouteProp<RootStackParamList, "UrgeLoop">;
...
const UrgeLoopScreen: React.FC = () => {
  const navigation = useNavigation<UrgeLoopNavigationProp>();
  const route = useRoute<UrgeLoopRouteProp>();
  const progressAnim = useRef(new Animated.Value(0)).current;
  const [selectedLoop, setSelectedLoop] = useState<string | null>(null);

  const questionNumber = route.params?.questionNumber ?? 20;
  ...
  const handleContinue = () => {
    ...
    // Navigate to PanicButtonInfo screen (question 21)
    navigation.navigate("PanicButtonInfo", {
      questionNumber: 21,
      ...
      <View style={styles.container}>
        ...
        <View style={styles.content}>
          <Text style={styles.heading}>
            Do you ever feel stuck in a loop where the urge takes over completely?
          </Text>

          <View style={styles.optionsContainer}>
            {loopOptions.map((option) => {
              const isSelected = selectedLoop === option;
              return (
                <TouchableOpacity
                  key={option}
                  style={[
        ...
        <View style={styles.footer}>
          <TouchableOpacity
            style={[
              ...
              !selectedLoop && styles.continueButtonDisabled,
            ]}
            activeOpacity={0.85}
