...
type SexualFantasyNavigationProp = StackNavigationProp<
  RootStackParamList,
  "SexualFantasy"
>;

type SexualFantasyRouteProp = RouteProp<
  RootStackParamList,
  "SexualFantasy"
>;
...
const SexualFantasyScreen: React.FC = () => {
  const navigation = useNavigation<SexualFantasyNavigationProp>();
  const route = useRoute<SexualFantasyRouteProp>();
  const progressAnim = useRef(new Animated.Value(0)).current;
  const [selectedFantasy, setSelectedFantasy] = useState<FantasyOption | null>(
    null
  );

  const questionNumber = route.params?.questionNumber ?? 14;
  ...
  const handleOptionSelect = (option: FantasyOption) => {
    setSelectedFantasy(option);
  };

  const handleContinue = () => {
    if (!selectedFantasy) return;

    // Navigate to RelationshipImpact screen (question 15)
    navigation.navigate("RelationshipImpact", {
      questionNumber: 15,
        ...
        <View style={styles.content}>
          ...
          <View style={styles.optionsContainer}>
            {fantasyOptions.map((option) => {
              const isSelected = selectedFantasy === option;
              return (
                <TouchableOpacity
                  key={option}
                  style={[
        ...
        <View style={styles.footer}>
          <TouchableOpacity
            style={[
              ...
              !selectedFantasy && styles.continueButtonDisabled,
            ]}
            activeOpacity={0.85}
            onPress={handleContinue}
            disabled={!selectedFantasy}
          >
...
export default SexualFantasyScreen;
