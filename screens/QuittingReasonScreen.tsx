...
type QuittingReasonNavigationProp = StackNavigationProp<
  RootStackParamList,
  "QuittingReason"
>;

type QuittingReasonRouteProp = RouteProp<
  RootStackParamList,
  "QuittingReason"
>;
...
const reasonOptions = [
  ...
  "I want healthier intimacy",
  "I don't know, just want to stop",
];

const QuittingReasonScreen: React.FC = () => {
  const navigation = useNavigation<QuittingReasonNavigationProp>();
  const route = useRoute<QuittingReasonRouteProp>();
  const progressAnim = useRef(new Animated.Value(0)).current;
  const [selectedReason, setSelectedReason] = useState<string | null>(null);

  const questionNumber = route.params?.questionNumber ?? 6;
  ...
  const handleContinue = () => {
    ...
    // Navigate to previous attempt screen (question 7)
    navigation.navigate("PreviousAttempt", {
      questionNumber: 7,
...
export default QuittingReasonScreen;
